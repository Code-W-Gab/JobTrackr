...
export default function Dashboard(){
  ...
  return(
    <main className="h-full w-full overflow-y-auto p-6 bg-[#f5f7f7] border-l border-indigo-100 dark:bg-[#161f2e] dark:border-gray-700">
      <div className="flex items-center justify-between">
        <div className="space-y-1">
          <h1 className="font-bold text-xl text-gray-800 dark:text-white">Good morning, {user?.fullName}</h1>
          <p className="text-xs text-gray-500 dark:text-gray-400">Here's what's happening with your job search today.</p>
        </div>
      ...
      <div className='grid grid-cols-6 gap-4 mt-8'>
        {status.map((stats, index) => {
          return(
            <div key={index} className='bg-white p-4 rounded-lg border border-slate-100 dark:bg-[#1E293B] dark:border-gray-700'>
              ...
              <div className='mt-2'>
                ...
                <p className='text-xs mt-1 text-gray-600 dark:text-gray-300'>{stats.name}</p>
              </div>
            </div>
          )
        })}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mt-4">
        ...
        <div className="lg:col-span-2 bg-white  rounded-2xl p-6 border border-slate-100 dark:bg-[#1E293B] dark:border-gray-700">
          <div className='mb-6'>
            <h3 className="font-semibold text-[#0F172A] dark:text-white">Applications Over Time</h3>
          ...
          <div className="flex items-center gap-4 mt-4">
            <div className="flex items-center gap-2"><div className="w-3 h-3 rounded-full bg-indigo-600" /><span className="text-xs text-slate-500 dark:text-slate-300">Applications</span></div>
            <div className="flex items-center gap-2"><div className="w-3 h-3 rounded-full bg-emerald-500" /><span className="text-xs text-slate-500 dark:text-slate-300">Interviews</span></div>
          </div>
        </div>
        ...
        <div className="bg-white  rounded-2xl p-6 border border-slate-100 dark:bg-[#1E293B] dark:border-gray-700">
          <div className="mb-6">
            <h3 className="font-semibold text-[#0F172A] dark:text-white ">Status Distribution</h3>
          ...
          <div className="space-y-2 mt-2">
            {STATUS_DISTRIBUTION.slice(0, 4).map(item => (
              <div key={item.name} className="flex items-center justify-between">
                ...
                <span className="text-xs font-medium text-[#0F172A] dark:text-white">{item.value}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className='grid grid-cols-3 gap-4'>
        {/* Upcoming Interviews */}
        <section className='bg-white p-4 mt-4 rounded-xl border border-slate-100 dark:bg-[#1E293B] dark:border-gray-700'>
          <div className='flex items-center justify-between'>
            <h1 className='font-semibold text-[#0F172A] dark:text-white'>Upcoming Interviews</h1>
            <OnNavigate to="/calendar">
              <button className='text-indigo-600 text-xs hover:underline cursor-pointer dark:text-indigo-400'>
                View all
              </button>
            </OnNavigate>
          </div>
            ...
            {upcomingInterviews.length === 0 ? (
              ...
              <div className='space-y-2 mt-4'>
                {upcomingInterviews.map((interview, index) => {
                  return(
                    <div key={interview._id} className='flex items-center justify-between py-2 px-3 bg-gray-50 hover:bg-gray-100 rounded-lg dark:hover:bg-gray-700 dark:bg-[#161f2e]'>
                      <div className='flex gap-2'>
                        ...
                        <div>
                          <h4 className='text-sm font-medium text-[#0F172A] dark:text-white'>{interview.companyName}</h4>
                          <p className='text-xs text-slate-500 dark:text-slate-400'>{interview.jobType}</p>
                        </div>
                      </div>
        ...
        {/* Recent Activity */}
        <section className='bg-white p-4 mt-4 rounded-xl border border-slate-100 dark:bg-[#1E293B] dark:border-gray-700'>
          <h1 className='font-semibold text-[#0F172A] dark:text-white'>Recent Activity</h1>
          <div className='mt-4'>
                  ...
                  return(
                    <div key={activity._id} className='flex items-center gap-2'>
                      ...
                      <div className='flex flex-col'>
                        <h3 className='text-sm text-slate-500 dark:text-slate-400'>{ActivityStatus[activity.status] || "Updated"} <span className='font-medium text-black dark:text-white'>{activity.companyName}</span></h3>
        ...
        {/* Quick Action */}
        <section className='bg-white p-4 mt-4 rounded-xl border border-slate-100 dark:bg-[#1E293B] dark:border-gray-700'>
          <h1 className='font-semibold text-[#0F172A] dark:text-white'>Quick Actions</h1>
          <div className="space-y-2 mt-4">
            {[
              ...
              <OnNavigate to={item.navigate} key={item.label} onClick={item.action} className='w-full flex items-center gap-3 p-3 rounded-xl bg-slate-50 hover:bg-slate-100 transition-all group dark:bg-[#161f2e] dark:hover:bg-slate-700'>
                <div className={`w-8 h-8 rounded-lg ${item.color} flex items-center justify-center shrink-0`}>
                  <Plus className="w-4 h-4 text-white" />
                </div>
                <div className="flex-1 text-left">
                  <div className="text-sm font-medium text-[#0F172A] dark:text-white">{item.label}</div>
      ...
      {/* Recent Application */}
      <div className="bg-white rounded-2xl border border-slate-100 overflow-hidden mt-4 dark:bg-[#1E293B] dark:border-gray-700">
        <div className="flex items-center justify-between p-5 border-b border-slate-100 dark:border-gray-700">
          <h3 className="font-semibold text-[#0F172A] dark:text-white">Recent Applications</h3>
          <OnNavigate to='/application'>
            <button className="flex items-center gap-1 text-xs text-indigo-600  hover:underline dark:text-indigo-500">
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-slate-50 dark:bg-[#161f2e] dark:border-gray-700">
              <tr>
                {['Company', 'Position', 'Date', 'Status'].map(h => (
                  <th key={h} className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider dark:text-slate-300">{h}</th>
                ))}
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-50 ">
              {recentApplications.map((application, index) => {
                return(
                  <tr className="hover:bg-slate-50/50 transition-colors dark:hover:bg-slate-700 dark:border-gray-700" key={application._id}>
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        <span className="text-sm font-medium text-[#0F172A] dark:text-white">{application.companyName}</span>
                      </div>
                    </td>
                    <td className="px-6 py-4 text-sm text-slate-600 dark:text-slate-300">{application.jobType}</td>
                    <td className="px-6 py-4 text-sm text-slate-500 dark:text-slate-400">{formatDateForInput(application.dateApplied)}</td>
                    <td className='px-6 py-4'>
